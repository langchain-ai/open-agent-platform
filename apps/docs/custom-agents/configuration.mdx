...
You can also configure Google AI models, such as Google Gemini. To do this, include an option like `{ label: "Google Gemini 1.5 Pro", value: "google/gemini-1.5-pro" }` (for TypeScript) or `{ "label": "Google Gemini 1.5 Pro", "value": "google:gemini-1.5-pro" }` (for Python) in the `options` list for `modelName`.

**Important:** When using Google AI models, ensure your agent's Python environment (on LangGraph Platform) has the `langchain-google-genai` package installed and the `GOOGLE_API_KEY` environment variable is set with your API key.
