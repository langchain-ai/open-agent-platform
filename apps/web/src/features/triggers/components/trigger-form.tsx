...
export function TriggerForm({ trigger, onCancel }: TriggerFormProps) {
  ...
  const emptyQueryParams = Object.entries(trigger.queryParams || {}).filter(([_, value]) => !value)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (responseData.authUrl) {
        ...
        toast.success(`${trigger.name} trigger has been registered successfully.`, {
          richColors: true
        })
      ...
      toast.error(`${trigger.name} trigger registration failed`, {
        description: error instanceof Error ? error.message : "Failed to register trigger",
        richColors: true
      })
    } finally {
      setIsLoading(false)
  ...
  const handleAuthComplete = () => {
    ...
    toast.success(`${trigger.name} trigger has been registered and authenticated successfully.`, {
      richColors: true
    })
    // Reset form and go back
    setFormData({})
  ...
  const renderFormField = (field: { name: string; type: string; required?: boolean; description?: string }) => {
    ...
    switch (field.type) {
        ...
        return (
          <div className="space-y-2">
            ...
            <Input
              ...
              type="number"
              value={value}
              onChange={(e) => handleInputChange(field.name, e.target.valueAsNumber || "")}
      ...
      default:
        if (
          (field.description && field.description.toLowerCase().includes("long")) ||
          ...
          {emptyQueryParams.length === 0 && payloadFields.length === 0 && (
            <div className="rounded-lg border border-dashed p-6 text-center">
              <p className="text-sm text-muted-foreground">
                No additional configuration required. Click the button below to register this trigger.
              </p>
            </div>
          )}
