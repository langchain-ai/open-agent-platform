...
import { ReactFlow, Edge, Controls } from "@xyflow/react";
...
export function AgentGraphVisualization({
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const subagents: any[] = configurable?.subagents || [];
    ...
    if (subagents.length > 0) {
      ...
      subagents.forEach((subagent, subagentIndex) => {
        ...
        const subagentTools = subagent?.tools || [];
        subagentTools.forEach((tool: string, toolIndex: number) => {
    ...
    if ((configurable?.subagents || []).length > 0) {
      (configurable.subagents as any[]).forEach((_, index: number) => {
        edges.push({
