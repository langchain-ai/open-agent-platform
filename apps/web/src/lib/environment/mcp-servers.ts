...
export function getMCPServer(
  ...
  console.warn(
    `Server ${serverName} uses STDIO transport which is not supported in browser environment`,
  );
  return undefined;
}
...
export function validateMCPServers(servers: MCPServersConfig): string[] {
  ...
  for (const [serverName, config] of Object.entries(servers)) {
    if (config.type === "stdio") {
      if (!config.command) {
        errors.push(
          `Server ${serverName}: STDIO transport requires 'command' field`,
        );
      }
      if (!Array.isArray(config.args)) {
        errors.push(
          `Server ${serverName}: STDIO transport requires 'args' to be an array`,
        );
      }
    } else if (config.type === "http" || config.type === "sse") {
      if (!config.url) {
        errors.push(
          `Server ${serverName}: HTTP/SSE transport requires 'url' field`,
        );
      } else {
        try {
          new URL(config.url);
      ...
      if (config.authProvider) {
        if (
          config.authProvider.type === "oauth" &&
          !config.authProvider.clientId
        ) {
          errors.push(
            `Server ${serverName}: OAuth auth provider requires 'clientId'`,
          );
        }
        if (
          config.authProvider.type === "api-key" &&
          !config.authProvider.apiKey
        ) {
          errors.push(
            `Server ${serverName}: API key auth provider requires 'apiKey'`,
          );
        }
      }
    } else {
      errors.push(
        `Server ${serverName}: Invalid transport type '${config.type}'`,
      );
    }
  }
